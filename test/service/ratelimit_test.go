...
func (this *rateLimitServiceTestSuite) setupBasicService() ratelimit.RateLimitServiceServer {
	...
	this.configLoader.EXPECT().Load(
		[]config.RateLimitConfigToLoad{{"config.basic_config", "fake_yaml", nil}},
		gomock.Any()).Return(this.config)

	// reset exporter before using
	testSpanExporter.Reset()

	return ratelimit.NewService(this.runtime, this.cache, this.configLoader, this.statsManager, true, MockClock{now: int64(2222)}, false, nil)
}
...
func TestService(test *testing.T) {
	...
	t.configLoader.EXPECT().Load(
		[]config.RateLimitConfigToLoad{{"config.basic_config", "fake_yaml", nil}}, gomock.Any()).Do(
...
func TestInitialLoadError(test *testing.T) {
	...
	service := ratelimit.NewService(t.runtime, t.cache, t.configLoader, t.statsManager, true, t.mockClock, false, nil)
